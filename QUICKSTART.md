...
- **JDK 21** (Temurin works; Maven itself is not needed â€” `mvnw` downloads it)
...
The server source ships in this repo at `server/` â€” luigeneric's BSGOCore imported with its
...
- `CLIENT_PATH` â€” full path to your `bsgo.exe` (used by `runclient.bat`)
- `GAMESERVER_IGNORE_HASHES=true` â€” your client's MD5 is not in the allowlist
...
The catalogue is generated, not shipped as JSON. One input comes from **your client** â€” the
localisation key list â€” because it is client-derived and is never committed to this repo.
...
bad card reach a client, because the client has no timeout on card loads â€” a single malformed
...
`size: N` is the card count actually parsed â€” if the line is missing, the catalogue never loaded.
...
argument â€” no client modification needed.
...
The one rule that saves the most time: **abrupt disconnect â†’ read the server log. Infinite
loading screen â†’ read the client's `output_log.txt`.** They almost never both have it.

| Symptom | Cause |
|---|---|
| Client exits to desktop the instant it connects, nothing in either log | Patch `0001` not applied â€” the client hardcodes protocol revision 4578 and calls `Application.Quit()` on mismatch |
| First login works, second hangs on "Loadingâ€¦ please wait a bit" forever | Patches `0002`/`0004` not applied â€” the session is consumed on connect and destroyed on disconnect |
| `size:` line missing at boot | `ServerConfigurationUtils/` missing or misnamed (step 2) |
| Loading screen spins forever, server log says `Card should not be send because it's null! <guid> <view>` | A card is missing from the catalogue â€” the log names it |
| Player never spawns into the sector, no error anywhere | Missing `ColliderTemplate` â€” the `config/` overlay was not copied (step 2) |
| Login works but nobody spawns into any sector; the log shows every sector thread throwing an uncaught exception at the same timestamp | A source file was edited (or `mvnw compile` ran) while the server was up â€” dev mode's hot reload kills the sector threads and leaves the login listener alive. Restart the server; never touch `server/src` while it runs |
| Abrupt disconnect, server log shows `Cannot invoke "java.lang.Float.floatValue()"` | A ship card is missing a stat â€” patch `0006` turns the crash into a default, and `cards.js` validates the full flight-stat set |
| `roll is NaN` every tick in the server log | Incomplete flight stats on a ship card â€” regenerate with `cards.js`, which validates this |
| Progress lost after a server restart | Patch `0007` not applied â€” upstream's clean shutdown wrote guilds only |
| A `patches/` file fails to apply to pristine upstream | Wrong baseline (`git checkout 23bad98a`) or missing `--ignore-whitespace` â€” only relevant when reviewing, the fork already carries them |
| Server does not boot, exception does not name a sector | A star in `galaxy.js` has no sector template on disk â€” run `emit-sector-templates.js`; validator V2a in `cards.js` catches this first and lists every missing id |
| Client build fails under `mvnw` | `JAVA_HOME` points at the wrong Java â€” it must be a JDK 21 |
| Every label shows its raw key (`BGO.HUB.MENU.SHIP_HANGAR`), client log says `Requested unknown asset 'locale_0.xml'` | The `+language` launcher argument matched no locale bundle (a typo like `e` for `en`) â€” the client loads an empty string table with no error. `runclient.bat` validates the code and falls back to `en` |
